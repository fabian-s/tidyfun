...
author: 
    ...
    - "Jeff Goldsmith$^\\dag$"
institute: 
    - "$\\star$: Dept. of Statistics, LMU Munich"
    - "$\\dag$: Columbia University Mailman School of Public Health"
output: 
  beamer_presentation: 
    keep_tex: yes 
    includes:
...
ggplot(medians) + geom_spaghetti(aes(y = median_rcst, col = sex, linetype = case), size = 1)
```
...
- an abstract superclass for functional data in 2 forms:
    - as (argument, value)-tuples: subclass **`tfd`** (regular, irregular)
    - or in basis representation: subclass **`tfb`** (splines, FPCs)
...
- basically, a `list` of numeric vectors  
  (... since `list`s work well as columns of data frames ...)
    - evaluations for `tfd`
    - basis coefficients for a joint basis for `tfb`
...
Functional data in basis representation: 
 ...
 - use either **`mgcv`**-spline bases or (F)PCs; wavelets to be added
...
```{r, eval = FALSE}
ex  %>% plot()
ex %>% tfb(k = 20, bs = "ps", m = c(2,1)) %>% plot(col = "red")
ex %>% tfb(k = 45, penalized = FALSE) %>% lines(col = "blue")
```
```{r, echo = FALSE, results = 'hide', out.height = '.4\\textheight', fig.width = 8, fig.height = 4}
layout(t(1:2))
plot(ex, alpha = 1)
plot(tfb(ex, k = 20, bs = "ps", m = c(2,1)), col = scales::alpha("red", 0.5),
  ylab="tfb(ex)")
lines(tfb(ex, k = 45, penalized = FALSE), col = scales::alpha("blue", 0.5))
```
<!--
## **`tf`** subclass: 
...
layout(t(1:2))
plot(ex, alpha = 1)
plot(ex  %>% tfb_fpc(smooth = FALSE, pve = .999), col = scales::alpha("red", .5),
  ylab = "tfb_fpc(ex)")
lines(ex %>% tfb_fpc(pve = .95), col = scales::alpha("blue", .5))
```
...
ex[1:2, seq(0, .15, l = 3), matrix = FALSE] %>% str
```
...
New **pasta-themed** `geom`s and plots for functional data:

- **`geom_spaghetti`** for lines
- **`geom_meatballs`**  for (lines &) points
- **`gglasagna`** with **`order`**-argument to sort the lasagna layers
...
```{r, eval = FALSE}
gglasagna(dti,cca, order = integrate(cca, definite = TRUE)) + 
  facet_wrap(~ case)
```
```{r, dti-fig2, echo = FALSE, out.height = '.7\\textheight', fig.width = 12, fig.height = 7}  
gglasagna(dti[1:100,], cca, order = integrate(cca, definite = TRUE), 
  arg = seq(0,1,l = 101)) + theme(axis.text.y = element_text(size = 6)) + 
