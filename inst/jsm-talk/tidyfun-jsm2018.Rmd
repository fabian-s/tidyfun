...
**`tfd`** objects contain "raw" functional data: 
 ...
 - a list of **`evaluations`** $f_i(t)|_{t=\boldmath{t'}}$ and corresponding **`arg`**s $\boldmath{t'}$
 - the **`domain`**: the range of valid **`arg`**s.
...
- a modifiable **`evaluator`** function that defines how to inter-/extrapolate `evaluations` between `arg`s (and remembers results of previous calls)
...
```{r, out.height = '.35\\textheight', fig.width = 4, fig.height = 4}
dti$rcst[1:2]
...
Functional data in basis representation: 
 ...
 - have an associated **`basis`** function that defines how to compute the basis for new **`arg`**s and how to differentiate/integrate.
